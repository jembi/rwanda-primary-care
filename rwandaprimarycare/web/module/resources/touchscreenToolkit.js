...
var messageCodes = [ 'touchscreen.next', 'touchscreen.back',
		'touchscreen.finish', 'touchscreen.skip', 'touchscreen.clear',
		'touchscreen.cancel', 'touchscreen.yes', 'touchscreen.no',
		'touchscreen.authorise', 'touchscreen.username',
		'touchscreen.showData', 'touchscreen.newPatient',
		'touchscreen.newGuardian', 'touchscreen.disable', 'touchscreen.enable',
		'touchscreen.year', 'touchscreen.month', 'touchscreen.hour',
		'touchscreen.minute', 'touchscreen.date', 'touchscreen.capturedDate',
		'touchscreen.usernameInvalid', 'touchscreen.wantToCancel',
		'touchscreen.enterValue', 'touchscreen.enterValidValue',
		'touchscreen.valueOutOfRange', 'touchscreen.valueTooSmall',
		'touchscreen.valueTooBig', 'touchscreen.selectValue',
		'touchscreen.not', 'touchscreen.ok', 'touchscreen.delete',
		'touchscreen.space', 'touchscreen.shift', 'touchscreen.unknown',
		'touchscreen.upper', 'touchscreen.lower', 'touchscreen.enterValidId' ];
...
function addLaunchButton() {
	if (document.forms.length > 0) {
		...
		launchButton.addEventListener("mousedown", toggleTouchscreenInterface,
				false);
		launchButton.addEventListener("click", toggleTouchscreenInterface,
				false);
...
function createInputPage(pageNumber) {
	...
	staticControls.setAttribute('class', 'staticControlsPage' + pageNumber
			+ ' ' + pageStyleClass)
...
function createButtons() {
	...
	buttonsDiv.innerHTML = "<button id='showDataButton' class='button gray navButton' onMouseDown='toggleShowProgress()'><span>"
			+ getMessage("touchscreen.showData") + "</span></button>";

	// create next/finish button
	buttonsDiv.innerHTML += "<button id='nextButton' class='button green navButton' onMouseDown='gotoNextPage()'><span>"
			+ getMessage("touchscreen.next") + "</span></button>";

	// create back button
	buttonsDiv.innerHTML += "<button id='backButton' class='button green navButton'><span>"
			+ getMessage("touchscreen.back") + "</span></button>";

	// create clear button or new patient button if on search page
	if (!tstSearchPage) {
		buttonsDiv.innerHTML += "<button id='clearButton' class='button gray navButton' onMouseDown='clearInput()'><span>"
				+ getMessage("touchscreen.clear") + "</span></button>";
	} else {
		...
		buttonsDiv.innerHTML += "<button id='newPatientButton' class='button navButton' onMouseDown='document.forms[0].submit()'><span>"
	...
	buttonsDiv.innerHTML += "<button class='button navButton red' id='cancelButton' onMouseDown='confirmCancelEntry();'><span>"
			+ getMessage("touchscreen.cancel") + "</span></button>";
	return buttonsDiv;
}
...
function getElementsByTagNames(parentNode, tagNames) {
	...
	if (!tagFound) {
		for ( var c = 0; c < parentNode.childNodes.length; c++) {
			var result = getElementsByTagNames(parentNode.childNodes[c],
					tagNames);
			if (!result)
				continue;
...
function getFormElements() {
	...
	for ( var i = 0; i < formElements.length; i++) {
		if (formElements[i].getAttribute("type") != "hidden"
...
function enableTouchscreenInterface() {
	...
	for ( var i = 0; i < numberOfInputElements; i++) {
		...
		if (tstFormElements[i].getAttribute("type") == "radio") {
			var selectOptions = document
					.getElementsByName(tstFormElements[i].name);
...
function populateInputPage(pageNum) {
	...
	$('tt_staticControlsWrapper').setAttribute('class',
			'tt_controls_' + identifier);
...
function getHelpText(inputElement, aPageNum) {
	...
	if (helpText.innerHTML == "") {
		...
		if (str != null) {
			...
			switch (str[3]) {
			/*
			 * case "1i": helpText.innerHTML = str[1]+" " +
			 * getMessage("touchscreen.year"); break; case "2i":
			 * helpText.innerHTML = str[1]+" " +
			 * getMessage("touchscreen.month");; break; case "3i":
			 * helpText.innerHTML = str[1]+" " +
			 * getMessage("touchscreen.date");; break;
			 */
			case "4i":
...
function getOptions() {
	...
	if (!tstSearchPage) {
		if (tstFormElements[i].getAttribute("ajaxURL") != null) {
			var completeAjaxURL = tstFormElements[i].getAttribute("ajaxURL")
					+ tstFormElements[i].value;
			...
			try {
				if (tstFormElements[i].name == 'COUNTRY') {
				...
				} else if (tstFormElements[i].name == 'PROVINCE'
						&& addressMap.country.nameId != null) {
					// needed for the edit screens to set
					// up prepopulated values
					...
					if (addressMap.country.nameId == ""
							&& addressMap.country.name != "") {
						setIDBasedOnPreviousId("COUNTRY",
								addressMap.country.name);
					}
					completeAjaxURL += "&previousValue="
							+ addressMap.country.nameId;
				} else if (tstFormElements[i].name == 'DISTRICT'
						&& addressMap.province.nameId != null) {
					// needed for the edit screens to set
					// up prepopulated values
					...
					if (addressMap.province.nameId == ""
							&& addressMap.province.name != "") {
						setIDBasedOnPreviousId("PROVINCE",
								addressMap.province.name);
					}
					// alert("addressMap.province.nameId:" +
					// addressMap.province.nameId);
					completeAjaxURL += "&previousValue="
							+ addressMap.province.nameId;
				} else if (tstFormElements[i].name == 'SECTOR'
						&& addressMap.district.nameId != null) {
					// needed for the edit screens to set
					// up prepopulated values
					...
					if (addressMap.district.nameId == ""
							&& addressMap.district.name != "") {
						setIDBasedOnPreviousId("DISTRICT",
								addressMap.district.name);
					}
					completeAjaxURL += "&previousValue="
							+ addressMap.district.nameId;
				} else if (tstFormElements[i].name == 'CELL'
						&& addressMap.sector.nameId != null) {
					// needed for the edit screens to set
					// up prepopulated values
					...
					if (addressMap.sector.nameId == ""
							&& addressMap.sector.name != "") {
						setIDBasedOnPreviousId("SECTOR", addressMap.sector.name);
					}
					completeAjaxURL += "&previousValue="
							+ addressMap.sector.nameId;
				} else if (tstFormElements[i].name == 'UMUDUGUDU'
						&& addressMap.cell.nameId != null) {

					if (addressMap.cell.nameId == ""
			...
			if (tstFormElements[i].tagName == "SELECT") {
				var selectOptions = tstFormElements[i]
				...
				if (tstFormElements[i].multiple)
					tstInputTarget.setAttribute("multiple", "multiple");
			} else if (tstFormElements[i].getAttribute("type") == "radio") {
				var selectOptions = document
						.getElementsByName(tstFormElements[i].name);
				for ( var j = 0; j < selectOptions.length; j++) {
					if (selectOptions[j].checked)
						tstInputTarget.value = selectOptions[j].value;
				}
				...
				if (tstFormElements[i].checked)
					tstInputTarget.value = "Yes";
				else
					tstInputTarget.value = "No";
			} else {
...
function scrollUp(element) {
	...
	scrollableDiv.scrollTop = scrollableDiv.scrollTop
...
function createProgressArea() {
	...
	progressArea.innerHTML = "<div id='progressAreaHeader' class='progressAreaHeader'>"
			+ +getMessage("touchscreen.capturedData") + "</div>";
	for ( var e = 0; e < numberOfPages; e++) {
		...
		inputIndex.innerHTML += (e + 1) + ". "
				+ getHelpText(tstFormElements[tstPages[e]], e).innerHTML
				+ ": <div class='progressInputValue'>" + tstPageValues[e]
				+ "</div>";
		if (e == currentPage) {
...
function loadSelectOptions(selectOptions, options) {
	...
	for ( var j = 0; j < selectOptionCount; j++) {
		...
		if (selectOptions[j].value) {
			optionsList += " id='option" + selectOptions[j].value
					+ "' tstValue='" + selectOptions[j].value + "'";
		}
		// njih
...
function loadOptions(selectOptions, options) {
	...
	var selectOptionCount = selectOptions.length;
	for ( var j = 0; j < selectOptionCount; j++) {
		optionsList += "<li onmousedown='updateTouchscreenInputForSelect(this)'>"
				+ selectOptions[j].value + "</li>\n";
	}
	optionsList += "</ul>";
	options.innerHTML = optionsList;
}
...
function elementToTouchscreenInput(aFormElement, aInputDiv) {
	var aTouchscreenInputNode = aInputDiv.appendChild(document
			.createElement("input"));
...
function updateTouchscreenInputForSelect(element) {
	...
	if (multiple) {
		...
		if (inputTarget.value.indexOf(tstMultipleSplitChar) == 0)
			inputTarget.value = inputTarget.value.substring(1,
		...
		if (options[i].style) {
			...
			if (optionIncludedInValue(unescape(options[i].innerHTML), val_arr)) {
				...
				if (options[i].getAttribute("tstValue")) {
					inputElement.setAttribute("tstValue", options[i]
							.getAttribute("tstValue"));
				}
			} else {
	...
	if (aXMLHttpRequest.readyState == 4 && aXMLHttpRequest.status == 200) {
		...
		if (optionsList.getElementsByTagName("li")[0] != null) {
			...
			for ( var i = 0; i < optionNodeCount; i++) {
				optionNodes[i].setAttribute("onmousedown",
...
function tt_update(sourceElement) {
	...
	var targetElement = returnElementWithAttributeValue("touchscreenInputID",
			sourceElement.getAttribute("refersToTouchscreenInputID"),
			tstFormElements);
	targetElement.focus();
			...
			if (isDateElement(targetElement) && (sourceValue.length > 0)
				...
				if (targetElement.getAttribute("multiple") == "multiple") {
					...
					for (i = 0; i < targetElement.options.length; i++) {
						if (optionIncludedInValue(
								targetElement.options[i].text, val_arr)) {
	...
	case "SELECT":
		 ...
		 * targetElement.options[i].selected = true; if
		 * (!targetElement.multiple) break; } else
		 * targetElement.options[i].selected = false; } break;
		 */
	}
}

function returnElementWithAttributeValue(attributeName, attributeValue,
		elementList) {
	var i;
...
function joinDateValues(aDateElement) {
	...
	if (str) {
		...
		// detect date
		var dayElement = document.getElementsByName(str[1] + '[' + str[2]
				+ '(3i)]')[0];
		if (!dayElement) {
			dayElement = document.getElementsByName(str[1]
					.substr(0, strLen - 4)
					+ 'date[' + str[2] + '(3i)]')[0];
		}
		...
		// detect month
		var monthElement = document.getElementsByName(str[1] + '[' + str[2]
				+ '(2i)]')[0];
		if (!monthElement) {
			monthElement = document.getElementsByName(str[1].substr(0,
					strLen - 4)
					+ 'month[' + str[2] + '(2i)]')[0];
		}
		if (monthElement) {
	...
	if (currentInput) {
		...
		if (currentPageIndex) {
			...
			currentPageIndex.innerHTML = (currentPage + 1)
					...
					+ progressAreaFormat(currentInput) + "</div>";
			currentPageIndex.removeAttribute("class");
		}
	}
	if (destPage < tstPages.length) {

		var condition = tstFormElements[tstPages[destPage]]
		...
		if (destPage + 1 == tstPages.length) {
			nextButton.innerHTML = "<span>" + getMessage("touchscreen.finish")
					+ "</span>";
		} else {
			nextButton.innerHTML = "<span>" + getMessage("touchscreen.next")
					+ "</span>";
		}
		...
		var requireNextClick = tstInputTarget
				.getAttribute("tt_requireNextClick");

		// Make sure the next button is setup for right defaults
		nextButton.setAttribute("onMouseDown", "gotoNextPage()");
		// if in fast mode and not retrospective mode and missing is not
		// disabled
		if (requireNextClick == "false") {
			if (tstRetrospectiveMode != "true") {
				...
				nextButton.innerHTML = "<span>"
						+ getMessage("touchscreen.skip") + "</span>"
			}
		}
...
function inputIsValid() {
	...
	if (validateResult.length > 0 && !tstSearchPage) {
		...
		if (ttInput.shouldConfirm) {
			message += " <a onmousedown='javascript:confirmValue()' href='javascript:;'>"
					+ getMessage("touchscreen.authorise") + "</a> ";
		}
		showMessage(message)
		var nextButton = tstNextButton;
		return false;
...
function confirmValue() {
	...
	confirmationBar.innerHTML += "<div style='display: block;'><input type='submit' value='"
			+ getMessage("touchscreen.ok")
			+ "' class='button' style='float: left;' onclick='validateConfirmUsername()' onmousedown='validateConfirmUsername()'/><input type='submit' value='"
			+ getMessage("touchscreen.cancel")
			+ "' class='button' style='float: right; right: 3px;' onmousedown='cancelConfirmValue()' />";
...
function confirmCancelEntry() {
	if (tstConfirmCancel) {
		tstMessageBar.innerHTML = getMessage("touchscreen.wantToCancel")
				+ "<br/>"
				+ "<button onmousedown='hideMessage(); cancelEntry();'><span>"
				+ getMessage("touchscreen.yes")
				+ "</span></button><button onmousedown='hideMessage();'><span>"
				+ getMessage("touchscreen.no") + "</span></button>";
...
function cancelEntry() {
	...
	window.location.href = window.tt_cancel_destination
			|| "/patient/menu?no_auto_load_forms=true";
}
...
function showBestKeyboard(aPageNum) {
	...
	if (isDateElement(inputElement)) {
		...
		if (tstSearchPage) {
			if (thisDate.isDayOfMonthElement())
				getDatePicker();
			else
	...
	if ((optionCount > 0 && optionCount < 6 && inputElement.tagName == "SELECT")
			|| (inputElement.getAttribute("multiple") == "multiple")) {
		$("keyboard").innerHTML = "";
		return;
	}
	...
	case "alpha":
		tsUserKeyboard == "QWERTY" ? $("keyboard").innerHTML = getQWERTYLowerKeyboard()
				: $("keyboard").innerHTML = getABCLowerKeyboard();

		break;
	case "upper":
		tsUserKeyboard == "QWERTY" ? $("keyboard").innerHTML = getQWERTYUpperKeyboard()
				: $("keyboard").innerHTML = getABCUpperKeyboard();
		break;
	case "lower":
		tsUserKeyboard == "QWERTY" ? $("keyboard").innerHTML = getQWERTYLowerKeyboard()
				: $("keyboard").innerHTML = getABCLowerKeyboard();
	...
	case "boolean":
		...
		break;
	case "id":
		tsUserKeyboard == "QWERTY" ? $("keyboard").innerHTML = getIDQWERTYKeyboard()
				: $("keyboard").innerHTML = getIDABCKeyboard(); // getIDKeyboard()
		break;
	case "disabled":
...
function isDateElement(aElement) {
	...
	/*
	 * // for date[year] dates if (aElement.name.search(/\[year\]/gi) != -1)
	 * return true;
	 * 
	 * if (aElement.id.search(/_day$/) != -1) return true; // for
	 * person[date(1i)] dates var re = /([^\[]*)\[([^\(]*)\(([^\)]*)/ig; var str =
	 * re.exec(aElement.name); if (str == null) { str = re.exec(aElement.name); //
	 * i don't know why we need this! } if (str == null) { return false; } else {
	 * return true; } //return (str[2] != null);
	 */
}
...
function addOption(optionText) {
	document.getElementById('selections').innerHTML += "<div name='option' class='selectionOption' onMouseDown='toggleMe(this)'>"
			+ optionText + "</div>";
}
...
function getQwertyKeyboard() {
	...
	keyboard.innerHTML += "<span class='qwertyKeyboard'>"
			+ "<span class='buttonLine'>" + getButtons("QWERTYUIOP")
			...
			+ getButtons("ZXCVBNM,.") + getButtonString('abc', 'A-Z')
			+ getButtonString('num', '0-9') + "</span>" + "</span>"
	return keyboard;
}

function getQWERTYUpperKeyboard() {
	// var keyboard = createKeyboardDiv();
	keyboard = "<span class='abcUpperKeyboard'>" + "<span class='buttonLine'>"
			+ getButtons('0123456789-') + "</span>"
			+ "<span class='buttonLine'>" + getButtons("QWERTYUIOP")
			+ getButtonString('backspace', getMessage("touchscreen.delete"))
			+ "</span><span class='buttonLine'>" + getButtons("ASDFGHJKL")
			+ getButtonString('apostrophe', "'")
			+ getButtonString('space', getMessage("touchscreen.space"))
			+ "</span><span class='buttonLine'>" + getButtons("ZXCVBNM")
			+ getButtonString('abcLower', getMessage("touchscreen.lower"))
			+ getButtonString('num', '0-9') + "</span>" +
...
function getQWERTYLowerKeyboard() {
	// var keyboard = createKeyboardDiv();
	keyboard = "<span class='abcLowerKeyboard'>" + "<span class='buttonLine'>"
			+ getButtons('0123456789-') + "</span>"
			+ "<span class='buttonLine'>" + getButtons("qwertyuiop")
			+ getButtonString('backspace', getMessage("touchscreen.delete"))
			+ "</span><span class='buttonLine'>" + getButtons("asdfghjkl")
...
function getABCUpperKeyboard() {
	...
	keyboard = "<span class='abcUpperKeyboard'>" + "<span class='buttonLine'>"
			...
			+ "</span><span class='buttonLine'>" + getButtons("IJKLMNOP")
			+ getButtonString('apostrophe', "'")
			+ getButtonString('space', getMessage("touchscreen.space"))
			+ "</span><span class='buttonLine'>" + getButtons("QRSTUVWXYZ")
			+ getButtonString('abcLower', getMessage("touchscreen.lower"))
			+ "</span>" +
			// "<span class='buttonLine'>" +
			// getButtons("!@#$%&-/,.?") +
			// "</span>" +
			"<span>" + getButtons('0123456789-') + "</span>" + "</span>";
	return keyboard;
}
...
function getNumericKeyboard() {
	var keyboard = "<span class='numericKeyboard'>"
			+ "<span id='buttonLine1' class='buttonLine'>" + getButtons("123")
			+ getCharButtonSetID("+", "plus")
			+ getCharButtonSetID("-", "minus")
			+ getCharButtonSetID("/", "slash")
			+ getCharButtonSetID("*", "star") + getButtonString('abc', 'A-Z')
			+ getButtonString('date', getMessage("touchscreen.date"))
			+ "</span><span id='buttonLine2' class='buttonLine'>"
			+ getButtons("456") + getCharButtonSetID("%", "percent")
			+ "</span><span id='buttonLine3' class='buttonLine'>"
			+ getButtons("789") + getCharButtonSetID("0", "zero")
...
function getIDABCKeyboard() {
	keyboard = "<span class='abcUpperKeyboard'>" + "<span class='buttonLine'>"
			+ getButtons('0123456789-') + "</span>"
			+ "<span class='buttonLine'>" + getButtons("ABCDEFGH")
			+ getButtonString('backspace', getMessage("touchscreen.delete"))
			+ "</span><span class='buttonLine'>" + getButtons("IJKLMNOP")
			+ "</span><span class='buttonLine'>" + getButtons("QRSTUVWXYZ")
			+ "</span>" +
...
function getDatePicker() {
	...
	if (railsDate.isDayOfMonthElement()) {
		getDayOfMonthPicker(railsDate.getYearElement().value, railsDate
				.getMonthElement().value);
		return;
	}
...
function getDayOfMonthPicker(aYear, aMonth) {
	...
	keyboard.innerHTML += getButtonString("Unknown",
			getMessage("touchscreen.unknown"));
	...
	/*
	 * if (aYear && aMonth) ds = new DateSelector({element: $("keyboard"),
	 * target: tstInputTarget, year: aYear, month: aMonth, format: "dd" }); else
	 * if (aMonth) ds = new DateSelector({element: $("keyboard"), target:
	 * tstInputTarget, month: aMonth, format: "dd" }); else ds = new
	 * DateSelector({element: $("keyboard"), target: tstInputTarget, format:
	 * "dd" });
	 */
}
...
function getCharButtonSetID(character, id) {
	return '<button onMouseDown="press(\'' + character
			+ '\');" class="keyboardButton" id="' + id + '"><span>' + character
			+ '</span></button>';
}

function getButtonString(id, string) {
	return "<button onMouseDown='press(this.id);' class='keyboardButton' id='"
			+ id + "'><span>" + string + "</span></button>";
}
...
function press(pressedChar) {
	...
	if (pressedChar.length == 1) {
		...
		switch (pressedChar) {
		case 'backspace':
			inputTarget.value = inputTarget.value.substring(0,
					inputTarget.value.length - 1);
			break;
		case 'done':
		...
		case 'id':
			tsUserKeyboard == "QWERTY" ? $("keyboard").innerHTML = getIDQWERTYKeyboard()
					: $("keyboard").innerHTML = getIDABCKeyboard();
			break;
		case 'date':
			getDatePicker();
			break;
...
function listSuggestions(inputTargetPageNumber) {
	...
	if (inputElement.getAttribute("ajaxURL") != null) {
		var completeAjaxURL = inputElement.getAttribute("ajaxURL")
				+ inputElement.value;
		...
		try {
			if (inputElement.name == 'COUNTRY') {
				completeAjaxURL += "&previousValue=" + "-1";
			} else if (inputElement.name == 'PROVINCE'
					&& addressMap.country.nameId != null) {
				completeAjaxURL += "&previousValue="
						+ addressMap.country.nameId;
			} else if (inputElement.name == 'DISTRICT'
					&& addressMap.province.nameId != null) {
				completeAjaxURL += "&previousValue="
						+ addressMap.province.nameId;
			} else if (inputElement.name == 'SECTOR'
					&& addressMap.district.nameId != null) {
				completeAjaxURL += "&previousValue="
						+ addressMap.district.nameId;
			} else if (inputElement.name == 'CELL'
					&& addressMap.sector.nameId != null) {
				completeAjaxURL += "&previousValue=" + addressMap.sector.nameId;
			} else if (inputElement.name == 'UMUDUGUDU'
					&& addressMap.cell.nameId != null) {
...
function matchOptions() {
	...
	for ( var i = 0; i < options.length; i++) {
		if (options[i].textContent.toLowerCase().indexOf(
				stringToMatch.toLowerCase()) == 0) {
			document.getElementById("selections").style.top = -i
					* selectionHeight + "px";
			return;
		}
	}
...
function enableValidKeyboardButtons() {
	...
	var validateUrl = "/cgi-bin/validate.cgi?p=" + patternStr + "&keys="
			+ availableKeys + "&s=" + inputElement.value;
...
var TTInput = function(aPageNum) {
	...
	if (isDateElement(this.formElement)) {
		this.formElement.value = this.element.value; // update date value
		// before validation so
		// we can use RailsDate
		var rDate = new RailsDate(this.formElement);
		this.value = rDate.getDayOfMonthElement().value + "/"
				+ rDate.getMonthElement().value + "/"
				+ rDate.getYearElement().value;
	}
...
TTInput.prototype = {
	...
	validateRange : function() {
		...
		if (isDateElement(this.formElement)) {
			...
			var thisDate = new Date(RegExp.$3, parseFloat(RegExp.$2) - 1,
					RegExp.$1);
			...
			// check for illegal numbers (also prevent ending values with a
			// period)
			if (isNaN(this.element.value)
					|| this.element.value.charAt(this.element.value.length - 1) == ".")
				return getMessage("touchscreen.enterValidValue");
			// check for floating points numbers
			if (this.element.value % 1 != 0
					&& isIntegerFieldType(this.element.getAttribute("name")))
				return getMessage("touchscreen.enterValidValue");
			...
			minValue = this.getNumberFromString(this.element
					.getAttribute("min"));
			maxValue = this.getNumberFromString(this.element
					.getAttribute("max"));
			absMinValue = this.getNumberFromString(this.element
					.getAttribute("absoluteMin"));
			absMaxValue = this.getNumberFromString(this.element
					.getAttribute("absoluteMax"));
		...
		if (tooSmall || tooBig) {
			if (!isNaN(minValue) && !isNaN(maxValue))
				return getMessage("touchscreen.valueOutOfRange") + ": "
						+ minValue + " - " + maxValue;
			if (tooSmall)
				return getMessage("touchscreen.valueTooSmall") + ": "
						+ minValue;
			if (tooBig)
				return getMessage("touchscreen.valueTooBig") + ": " + maxValue;
		}
		return "";
	},
	...
	validateSelectOptions : function() {
		...
		var allowFreeText = this.formElement.getAttribute("allowFreeText")
				|| "false";
		...
		if (tagName == "SELECT" || suggestURL != "" && allowFreeText != "true") {
			...
			if (!isAValidEntry)
				return getMessage("touchscreen.selectValue")
						+ " ("
						+ getMessage("touchscreen.not")
						+ ": "
						+ this.element.value
						+ ") <a onmousedown='javascript:confirmValue()' href='javascript:;'>"
						+ getMessage("touchscreen.authorise") + "</a>";
...
RailsDate.prototype = {
	...
	getDayOfMonthElement : function() {
		...
		if (str == null) {
			str = re.exec(this.element.name); // i don't know why we need
			// this!
		}
		if (str) {
			var strLen = str[1].length;
			...
			if (!dayElement) { // check name_date[name(3i)]
				if (str[1].search(/year$/) != -1) {
					elementName = str[1].replace(/year$/, "date") + "["
							+ str[2] + '(3i)]';
					dayElement = document.getElementsByName(elementName)[0];
				} else if (str[1].search(/month$/) != -1) {
					elementName = str[1].replace(/month$/, "date") + '['
							+ str[2] + '(3i)]';
					dayElement = document.getElementsByName(elementName)[0];
				}
			}
	...
	getMonthElement : function() {
		...
		if (str) {
			...
			if (!monthElement) { // name_month[name(2i)]
				if (str[1].search(/year$/) != -1) {
					elementName = str[1].replace(/year$/, "month") + '['
							+ str[2] + '(2i)]';
					monthElement = document.getElementsByName(elementName)[0];
				} else if (str[1].search(/date$/) != -1) {
					elementName = str[1].replace(/date$/, "month") + '['
							+ str[2] + '(2i)]';
					monthElement = document.getElementsByName(elementName)[0];
				}
			}
			...
			if (!monthElement) { // name[name(2i)]
				if (this.isYearElement()
						&& (this.element.name == str[1] + '[' + str[2]
								+ '(1i)]')) {
					monthElement = document.getElementsByName(str[1] + '['
							+ str[2] + '(2i)]')[0];
	...
	getYearElement : function() {
		...
		if (str) {
			...
			if (!yearElement) {
				if (str[1].search(/month$/) != -1) {
					elementName = str[1].replace(/month$/, "year") + '['
							+ str[2] + '(1i)]';
				} else if (str[1].search(/date$/) != -1) {
					elementName = str[1].replace(/date$/, "year") + '['
							+ str[2] + '(1i)]';
				}
				yearElement = document.getElementsByName(elementName)[0];
			}
			...
			if (!yearElement) {
				yearElement = document.getElementsByName(str[1] + '[' + str[2]
						+ '(1i)]')[0];
			}
