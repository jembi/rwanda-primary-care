...
<c:if test="${results == null}">
	<form method="get">
		...
		<tr>
			...
			<td><touchscreen:textInput  required="true"  label="${rwandanName}" name="RWNAME" fieldType="upper" value="${search}" allowFreeText="true" ajaxURL="findPatientByNameAjax.form?searchType=RWNAME&search="/></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				...
				<select optional="false" name="GENDER" label="${gender}" helpText="${gender}">
...
<tr>
			...
			<td><touchscreen:textInput  required="false" label="${CountryStr}" name="COUNTRY" value="Rwanda" allowFreeText="false" ajaxURL="findPatientByNameAjax.form?searchType=COUNTRY&search="  javascriptAction="updateAddressHierarchyCache()"/></td>
		</tr>
		<tr>
...
<touchscreen:patientList patients="${results}" maxResults="5" separator="" href="findPatientByNameConfirm.list" showAllIds="true"/>
	...
	<c:url var="extendHref" value="extendNameSearch.form">
		<c:param name="addIdentifier" value="${addIdentifier}"/>
		<c:param name="givenName" value="${param.FANAME}"/>
		<c:param name="familyName" value="${param.RWNAME}"/>
		<c:param name="gender" value="${param.GENDER}"/>
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<c:if test="${CR == null}">
	<touchscreen:button label="Extend Search To Client Registry ${param.FANAME}" href="${extendHref}"/>
	</c:if>
</c:if>
